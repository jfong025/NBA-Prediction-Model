...
def clean_moneyline_data(df):
    ...
    #moneyline_df['away_moneyline1'] = moneyline_df['away_moneyline'].apply(lambda x: x[0]).apply(lambda x: x.find('-') == -1)
    away_mls = df['away_moneyline'].str.split(',', expand=True).iloc[:, [0,1,2,3]]
    print(away_mls)
    away_mls.columns = ['away_ml1', 'away_ml2', 'away_ml3', 'away_ml4']
    home_mls = df['home_moneyline'].str.split(',', expand=True).iloc[:, [0,1,2,3]]
